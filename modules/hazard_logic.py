...
def get_weather_element(wx_code: int, wind_spd: float) -> str:
    """Translates WMO weather codes into human-readable text.

    Includes synthetic codes 68/69 generated by the ARMS Thermal Phase Gate
    for freezing-rain-in-transition-zone scenarios.
    """
    wx_map = {
        ...
        # --- ARMS Synthetic Codes (Thermal Phase Gate) ---
        68: "Light Freezing Rain (Transition Zone)",
        69: "Heavy Freezing Rain (Transition Zone)",
...
def get_turb_ice(
    ...
    alt_temp: float,
    alt_rh: int,
    terrain_type: str = "Land",
    cloud_base_agl: float = 10000,
) -> tuple[str, str]:
    """
    ...
    if alt > 0:
        if t_val > 0 or t_val < -40:
            ...
            if not (in_cloud or has_liquid_precip or is_wet_snow):
                ...
                if wx_val in [66, 67, 68, 69, 95, 96, 99]:
                    ice = "SEV CLR"
...
def hazard_lvl(h_str: str) -> float:
    """Converts a hazard string to a numeric severity level for Go/No-Go comparison.

    FIX: Previous ordering checked "SEV" before "MOD-SEV", causing MOD-SEV
    to always return 3.0 (SEV). The correct order is longest-match-first.
    """
    h_str = h_str.upper()
    if "MOD-SEV" in h_str: return 2.5   # ← Must precede "SEV" and "MOD"
    if "SEV"     in h_str: return 3.0
    if "MOD"     in h_str: return 2.0
    if "LGT"     in h_str: return 1.0
    return 0.0


def apply_tactical_highlights(text: str) -> str:
    """
