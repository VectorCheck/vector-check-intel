...
st.markdown("""
    ...
    /* Match METAR/TAF font to the Hazard Stack/Table font */
...
def apply_tactical_highlights(text):
    ...
    def vis_match(m):
        try:
            ...
            val = float(parts[0]) + float(eval(parts[1])) if ' ' in s else (float(eval(s)) if '/' in s else float(s))
    ...
    try:
        ...
        if m_res.get('data'):
            for report in m_res['data']:
                raw = apply_tactical_highlights(report.get('raw_text', ''))
                if "SPECI" in raw: raw = raw.replace("SPECI", '<span style="color: #E58E26; font-weight: bold;">SPECI</span>')
                metars.append(raw)
        ...
        return "<br>".join(metars) if metars else "STATION INACTIVE", taf_final
    except: return "CONNECTION ERROR", "CONNECTION ERROR"

@st.cache_data(ttl=600)
def fetch_mission_data(latitude, longitude, model_url, time_key):
    p_levels = [1000, 950, 925, 900, 850, 800, 700, 600, 500, 400]
    hourly_params = ["temperature_2m", "relative_humidity_2m", "wind_speed_10m", "wind_gusts_10m",
                     "wind_direction_10m", "weather_code", "wind_speed_80m", "wind_speed_120m", 
                     "freezing_level_height"]
    hourly_params += [f"temperature_{p}hPa" for p in p_levels] + [f"dewpoint_{p}hPa" for p in p_levels]
    
    params = {"latitude": latitude, "longitude": longitude, "hourly": hourly_params, 
              "wind_speed_unit": "kn", "forecast_hours": 48, "timezone": "UTC"}
    try:
...
if data and "hourly" in data:
    ...
    temp, hum, wx_code = h['temperature_2m'][idx], h['relative_humidity_2m'][idx], h['weather_code'][idx]
    ...
    cols[2].metric("Wind Dir", f"{str(int(h['wind_direction_10m'][idx])).zfill(3)}°")
    cols[3].metric("Wind Spd", f"{int(h['wind_speed_10m'][idx])} kt")
    cols[4].metric("Precip Type", get_precip_type(wx_code))
    cols[5].metric("Vis (Est)", f"{int((100 - hum) / 5 * 1.13)} sm")
    cols[6].metric("Freezing LVL", frz_display)
    cols[7].metric("Cloud Base", f"{int((temp - (temp - ((100-hum)/5)))*122*3.28)} ft")

    st.subheader("Tactical Hazard Stack")
    gst, w_spd = h['wind_gusts_10m'][idx], h['wind_speed_10m'][idx]
    upper_v, upper_h = (h['wind_speed_120m'][idx], 120) if h.get('wind_speed_120m') and h['wind_speed_120m'][idx] else (h.get('wind_speed_80m', [0]*idx)[idx], 80)
    
    if w_spd and upper_v and gst:
        stack = []
    ...
    st.error("Model Data Unavailable: ECMWF provides lower resolution vertical data. Please try HRDPS for Skew-T.")
